...
import com.grasset.exception.NoSuchModuleException;
...
public class ModuleServiceImpl implements ModuleService {
    ...
    public Module getModule(Scope scope, String moduleName) throws Exception {
        ...
        Path modulePath = PathHelper.getModulePath(scopeName, moduleName);

        if (moduleExists(modulePath)) {

            Path metaFilePath = PathHelper.getModuleMetaFilePath(scopeName, moduleName);
    ...
    public Module renameModule(Scope scope, Module module, String newName) throws Exception {
        try {
            ...
            if (moduleExists(modulePath)) {
                Path newModulePath = PathHelper.getModulePath(scopeName, newName);
                Files.move(modulePath, newModulePath);

                Path metaFileName = PathHelper.getModuleMetaFilePath(scopeName, newName, moduleName);
                Path newMetaFilePath = PathHelper.getModuleMetaFilePath(scopeName, newName, newName);
    ...
    public void addDocumentation(Scope scope, Module module, String docDirectory) throws Exception {
        ...
        if (moduleExists(modulePath)) {
            ...
            if (metaFileExists(metaFilePath)) {
                ...
                Path newDocPath = PathHelper.getModuleDocumentationPath(scopeName, moduleName, nextVersion);
